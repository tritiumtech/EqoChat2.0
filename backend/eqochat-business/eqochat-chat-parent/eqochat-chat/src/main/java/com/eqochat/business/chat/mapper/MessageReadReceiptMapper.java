...
public interface MessageReadReceiptMapper extends BaseMapper<MessageReadReceipt> {
    ...
    @Select("SELECT * FROM message_read_receipt WHERE reader_id = #{readerId} AND del_token = '0'")
    List<MessageReadReceipt> findByReaderId(@Param("readerId") Long readerId);
    
    @Select("SELECT * FROM message_read_receipt WHERE message_id = #{messageId} AND reader_id = #{readerId} AND del_token = '0' LIMIT 1")
