...
CREATE TABLE message_reaction (
    ...
    INDEX idx_reaction_del_token (del_token)
...
INSERT INTO user_profile (id, did, phone, email, nickname, status, credit_score, create_at, update_at) VALUES
(1, 'did:eqochat:system', '13800000000', 'system@eqochat.com', 'System', 'ACTIVE', 100, UNIX_TIMESTAMP(NOW()) * 1000, UNIX_TIMESTAMP(NOW()) * 1000);

INSERT INTO system_config (config_key, config_value, description, create_at, update_at) VALUES
('platform.name', 'EqoChat', '平台名称', UNIX_TIMESTAMP(NOW()) * 1000, UNIX_TIMESTAMP(NOW()) * 1000),
('platform.version', '2.0.0', '平台版本', UNIX_TIMESTAMP(NOW()) * 1000, UNIX_TIMESTAMP(NOW()) * 1000),
('user.default_credit_score', '50', '新用户默认信用分', UNIX_TIMESTAMP(NOW()) * 1000, UNIX_TIMESTAMP(NOW()) * 1000),
('agent.default_permission_level', 'L2', '新智能体默认权限等级', UNIX_TIMESTAMP(NOW()) * 1000, UNIX_TIMESTAMP(NOW()) * 1000),
('message.max_length', '5000', '单条消息最大字符数', UNIX_TIMESTAMP(NOW()) * 1000, UNIX_TIMESTAMP(NOW()) * 1000),
('group.max_members', '500', '群组最大成员数', UNIX_TIMESTAMP(NOW()) * 1000, UNIX_TIMESTAMP(NOW()) * 1000);
